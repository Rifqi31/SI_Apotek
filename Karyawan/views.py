...
def daftar_hadir_karyawan(request):
    ...
    if request.method == 'POST':
        ...
        daftar_hadir = Absen_karyawan.objects.filter(
            waktu__year=tahun, waktu__month=bulan,
...
def cetak_absensi(request, bulan, tahun):
    ...
    data = Absen_karyawan.objects.filter(
        waktu__year=tahun,
        waktu__month=bulan,
        karyawan__id=request.session['karyawan_id']
    )

    table_data = [["Tanggal", "Status"]]
    for x in data:
    ...
    content = [
        ...
        Spacer(1, 12),
        Paragraph(
            'Berikut ini adalah hasil rekam jejak kehadiran Anda selama bulan %s tahun %s:' % (bulan, tahun),
            styles['Normal']
        ),
        Spacer(1, 12),
        kehadiran_table,
        Spacer(1, 36),
        Paragraph('Mengetahui,', styles['Normal']),
        Spacer(1, 48),
        Paragraph('Rifqi Muttaqin, Head of Departement PT.iDEA Development.', styles['Normal'])
    ]
